        ...
        <div key={comment.id}>
          <div className="flex flex-col md:flex-row">
            <div className="flex flex-col items-center mb-5 mx-auto md:border-r-2 md:border-gray-300 w-full max-w-[13rem] md:min-h-64">
              <img
                src={(comment.politicalFigure.media as any).find((media: { key: string; }) => media?.key === 'featured')?.url ? `${url((comment.politicalFigure.media as any).find((media: { key: string; }) => media?.key === 'featured')?.url, 'media')}` : 'https://placehold.co/375'}
                alt="profile"
